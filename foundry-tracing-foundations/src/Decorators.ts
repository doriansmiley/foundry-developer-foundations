...
export function Trace(opts: TraceOptions) {
    return function (_target: any, _prop: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = async function (...args: any[]) {
            ...
            const resource: ResourceModel = { resource_id: uuidv4(), ...opts.resource };

            // attach context for child spans
            (this as any)[TRACE_CONTEXT] = { resource, traceId, spanId } as TraceContext;
...
export function TraceSpan(opts: ChildTraceOptions) {
    return function (_target: any, _prop: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = async function (...args: any[]) {
            ...
            if (!ctx) throw new Error('@TraceSpan requires a preceding @Trace on the instance');
            //OTLP‑compliant trace_id (32 hex digits) and span_id (16 hex digits)
