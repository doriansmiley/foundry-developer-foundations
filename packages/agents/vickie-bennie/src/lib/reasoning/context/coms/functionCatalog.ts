    ...
    [
      'getAvailableMeetingTimes',
      {
        ...
        implementation: async (
          ...
          const payload = getPayload(context, result);
          console.log(
            `getAvailableMeetingTimes returned: ${JSON.stringify(result)}`
          );
        ...
        >([
          [
            ...
            (context: Context, event: MachineEvent) => {
              console.log(
                `evaluating pause transition logic for state: ${event.stateId}`
              );
              if (event.payload && event.payload.stateId) {
                // if all are available return false and move to schedule meeting
                return !(event.payload[event.payload.stateId]
                  .allAvailable as boolean);
              }
              return true;
            },
          ],
    ...
    [
      'resolveUnavailableAttendees',
      {
        ...
        implementation: async (
          ...
          const result = await resolveUnavailableAttendees(
            context,
            event,
            task
          );
          const payload = getPayload(context, result);
          console.log(
            `resolveUnavailableAttendees returned: ${JSON.stringify(result)}`
          );
