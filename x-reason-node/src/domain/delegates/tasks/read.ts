import { FoundryClient, Tickets } from "@xreason/types";

export async function readTicket(id: string, client: FoundryClient): Promise<Tickets> {
    ...
    if (apiResponse.errorCode) {
        ...
        throw new Error(`An error occurred while calling read ticket errorInstanceId: ${apiResponse.errorInstanceId} errorCode: ${apiResponse.errorCode}`);
    }

    console.log(`the ticket execution ontology returned: ${JSON.stringify(apiResponse)}`)
