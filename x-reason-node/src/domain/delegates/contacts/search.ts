import { Contacts, FoundryClient, RfpRequests } from "@xreason/types";

export async function searchContacts(fullName: string, company: string, client: FoundryClient, pageSize = 10): Promise<Contacts[]> {
    ...
    if (apiResponse.errorCode) {
        ...
        throw new Error(`An error occurred while calling search Contacts errorInstanceId: ${apiResponse.errorInstanceId} errorCode: ${apiResponse.errorCode}`);
    }
