import { EIAResponse, GasScenarioResult } from "@xreason/types";

async function fetchCaGasPriceFromEIA(): Promise<{ date: string; price: number }> {
...
export function getVegaGasTrackerData(results: GasScenarioResult[]) {
    ...
    return {
        ...
        "mark": "bar",
        "encoding": {
            "x": { "field": "scenario", "type": "ordinal", "title": "Scenario Price ($/gal)" },
            "y": { "field": "annualCost", "type": "quantitative", "title": "Annual Incremental Cost ($B)" },
            "tooltip": [
                { "field": "scenario", "type": "ordinal", "title": "Scenario Price ($/gal)" },
                { "field": "annualCost", "type": "quantitative", "title": "Annual Cost ($B)" },
