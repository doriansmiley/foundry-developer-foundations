import { MeetingRequest, DerivedWindow } from "@xreason/types";
...
export function deriveWindowFromTimeframe(
    ...
    const { timeframe_context, working_hours, duration_minutes, localDateString } = req;
    ...
    switch (timeframe_context) {
        case 'user defined exact date/time': {
            ...
            const candidateStr = (localDateString && localDateString.trim()) ? localDateString : req.timeframe_context;
            const candidate = new Date(candidateStr);
            if (isNaN(candidate.getTime())) {
                throw new Error(`Invalid localDateString for exact meeting time: "${candidateStr}"`);
            }
            ...
            const clampedEnd = new Date(Math.max(clampedStart.getTime() + duration_minutes * 60_000, windowEndLocal.getTime()));
...
function nowInTZ(tz: string, ref: Date): Date {
    ...
    return new Date(Number(p.year), Number(p.month) - 1, Number(p.day), Number(p.hour), Number(p.minute), Number(p.second), 0);
}
...
function clampToWorkingInstant(d: Date, hours: { start_hour: number; end_hour: number }): Date {
    let x = new Date(d);
...
function endOfCurrentOrNextWorkWeek(start: Date, hours: { end_hour: number }): Date {
