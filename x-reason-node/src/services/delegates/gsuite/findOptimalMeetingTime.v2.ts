...
export async function findOptimalMeetingTimeV2({
    ...
    for (const calId in calendars) {
        ...
        for (const p of periods) {
            if (p.start && p.end && !isNaN(new Date(p.start).getTime()) && !isNaN(new Date(p.end).getTime())) {
...
export function toZonedISOString(utc: Date, tz: string, fallback = -420): string {
    try {
        ...
        return `${local.getFullYear()}-${padN(local.getMonth() + 1)}-${padN(local.getDate())}` +
            `T${padN(local.getHours())}:${padN(local.getMinutes())}:${padN(local.getSeconds())}` +
...
function buildWorkingWindowsUTC(
    ...
    for (let d = new Date(dayStartLocal); d <= lastLocalDay; d.setDate(d.getDate() + 1)) {
        const dow = d.getDay();
        ...
        const boundedStart = new Date(Math.max(startLocal.getTime(), windowStartLocal.getTime()));
        const boundedEnd = new Date(Math.min(endLocal.getTime(), windowEndLocal.getTime()));
