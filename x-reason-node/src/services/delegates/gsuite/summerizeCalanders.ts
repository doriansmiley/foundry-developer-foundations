...
import { CalendarSummary, EventSummary, ListCalendarArgs, Summaries } from '@xreason/types';
...
async function fetchCalendar(
    ...
    do {
        ...
        (res.data.items ?? []).forEach(evt => {
            ...
            const dur =
                (new Date(evt.end.dateTime).getTime() - new Date(evt.start.dateTime).getTime()) /
                60000;
