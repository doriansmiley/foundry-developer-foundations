 ...
 */
function getTimeZoneOffset(timeZone: string, date: Date, fallbackOffset?: number): number {
    try {
...
function calculateMeetingTime(
    ...
    const nowLocalString = new Date(now).toLocaleString("en-US", { timeZone: timezone });
    ...
    const adjustedStartTime = new Date(startTime.getTime() - offsetMinutes * 60 * 1000);
    const adjustedEndTime = new Date(endTime.getTime() - offsetMinutes * 60 * 1000);

    return { startTime: adjustedStartTime, endTime: adjustedEndTime };
}

function calculateTimeSlotScore(slot: TimeSlot, allBusyTimes: BusyPeriod[]): number {
    let score = 100;
    const hoursFromNow = (new Date(slot.start).getTime() - new Date().getTime()) / (1000 * 60 * 60);
    ...
    for (const busy of allBusyTimes) {
        ...
        const bufferBefore = (slotStart.getTime() - busyEnd.getTime()) / (1000 * 60);
...
function slotsOverlap(slot1: { start: string; end: string }, slot2: { start: string; end: string }): boolean {
    return new Date(slot1.start) < new Date(slot2.end) && new Date(slot2.start) < new Date(slot1.end);
}
...
export async function findOptimalMeetingTime(calendar: calendar_v3.Calendar, context: OptimalTimeContext): Promise<FindOptimalMeetingTimeOutput> {
    ...
    try {
        ...
        const { startTime, endTime } = calculateMeetingTime(context.timeframe_context, workingHours, new Date(), context.timezone);
        ...
        for (const participant of participants) {
            ...
            events.forEach(event => {
                if (event.status !== "cancelled" && event.start && event.end && event.start.dateTime && event.end.dateTime) {
                    const duration = calculateEventDuration(event.start.dateTime, event.end.dateTime)
                    if (duration > 0) {
                        eventTimeSlots.push({
        ...
        const filteredFreeBusySlots = busySlots.filter(fbSlot => {
            return !eventTimeSlots.some(eventSlot =>
                slotsOverlap({ start: eventSlot.start, end: eventSlot.end }, { start: fbSlot.start, end: fbSlot.end })
            );
        })
        ...
        const allBusyTimes: BusyPeriod[] = [
            ...
            if (seen.has(`${new Date(slot.start).toString()}-${new Date(slot.end).toString()}`)) {
                return false;
            }
            seen.add(`${new Date(slot.start).toString()}-${new Date(slot.end).toString()}`);
            return true;
        });
        ...
        while (currentTime < lastTime) {
            if (localHour >= workingHours.start_hour && localHour < workingHours.end_hour) {
                ...
                if (isAvailable) {
                    console.log(
                        `${LOG_PREFIX} isAvailable: ${isAvailable} currentTime: ${currentTime.toString()} toISOStringWithTimezoneL ${toISOStringWithTimezone(currentTime, context.timezone!, -420)}`
                    );
                    const slot: TimeSlot = {
                        start: toISOStringWithTimezone(currentTime, context.timezone!, -420),
        ...
        console.error(`${LOG_PREFIX} Finding optimal meeting time failed:\n  message: ${error instanceof Error ? error.message : error}\n  stack: ${error instanceof Error ? error.stack : ''}`, {
