...
import {
    ...
    fbFridayCalendars,
    fbDSTCalendars,
} from '@xreason/__fixtures__/schedule';

import {
    findOptimalMeetingTimeV2,
    Slot,
} from '@xreason/services/delegates/gsuite/findOptimalMeetingTime.v2';

// ------------------------------
// Inline mocks (auth INCLUDED)
// ------------------------------
let currentCalendarsFixture: Record<string, { busy: Array<{ start: string; end: string }> }> =
    fbSingleDayCalendars;
...
describe('findOptimalMeetingTimeV2 (plane-sweep + strict PT handling)', () => {
    ...
    it('DST spring-forward (Mar 7–10, 2025): shows -08:00 before and -07:00 after', async () => {
        ...
        expect(slots.length).toBeGreaterThan(0);
        const hasMinus08 = slots.some(s => /-08:00$/.test(s.start) || /-08:00$/.test(s.end)); // Fri
        const hasMinus07 = slots.some(s => /-07:00$/.test(s.start) || /-07:00$/.test(s.end)); // Mon
 ...
 */
function assertNoOverlap(slots: Slot[], busy: Array<{ start: string; end: string }>) {
