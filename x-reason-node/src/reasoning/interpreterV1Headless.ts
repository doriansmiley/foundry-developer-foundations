...
import { ActionType, programV1, MachineEvent, Context, StateConfig, Task } from ".";
...
export default function headlessInterpreter(
    ...
    const serialize = (state: State<Context, MachineEvent> | undefined) => state ? JSON.stringify(state) : '{}';
    ...
    const start = () => {
        ...
        if (state && retriggerEntryFunction) {
            ...
            const startingStateConfig = machine.config?.states?.[state.value as string];
            if (startingStateConfig?.entry && typeof startingStateConfig?.entry === 'function') {
                console.log(startingStateConfig.entry);
                (startingStateConfig.entry as (context: Context, event: MachineEvent) => void)(state.context!, { type: 'xstate.init' });
            }
        }
    };
