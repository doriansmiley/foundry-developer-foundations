...
async function getSolverTrainingData() {
  ...
  const searchResults = await trainingDataDao.search(SupportedEngines.SALES, SupportTrainingDataTypes.SOLVER);
...
async function getProgrammingTrainingData() {
  const trainingDataDao = container.get<TrainingDataDao>(TYPES.TrainingDataDao);
  const searchResults = await trainingDataDao.search(SupportedEngines.SALES, SupportTrainingDataTypes.PROGRAMMER);
  const trainingExamples = searchResults
    .reduce((acc, cur) => {
      acc = `${acc}
      If the task list is:
